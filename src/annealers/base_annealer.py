...
tmp_exp_path = Path('/home/bsulyok/documents/projects/gra/tmp_experiments')
...
def generate_experiment(graph_name, initial_embedding='random', exp_path=None, annealer_name=None):
    graph_data_path = data_root / graph_name
    assert graph_data_path.exists()
    uid = uuid.uuid4().hex
    if exp_path is None:
        exp_path = tmp_exp_path / uid
...
class BaseAnnealer:
    def __init__(self, conf_path=None, graph_name=None, initial_embedding=None):
        if conf_path is None or not conf_path.exists():
            conf_path = generate_experiment(graph_name=graph_name, initial_embedding=initial_embedding, annealer_name=self.__class__.__name__)
    ...
    def embed(self, steps, verbose=False):
