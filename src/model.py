...
def select_for_move_random(gd: np.ndarray) -> np.ndarray:
    return np.random.randint(gd.shape[0])
...
def embed(graph: nx.Graph, coord: np.ndarray = None, steps: int = 2**8, verbose: bool = False):
    adjacency_matrix = nx.adjacency_matrix(graph).todense().astype(bool)
    ...
    for step, temp in enumerate(temperature, start=1):
        print(grs)
        moved_vertex = select_for_move_random(gd)
